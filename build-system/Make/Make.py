...
class BazelCommandLine:
    ...
    def set_sideload_build(self, value):
        self.sideload_build = value
    ...
    def get_define_arguments(self):
        ...
        if self.disable_extensions:
            args += ['--//Telegram:disableExtensions']
        if self.sideload_build:
            args += ['--//Telegram:sideloadBuild']
...
def resolve_configuration(base_path, bazel_command_line: BazelCommandLine, arguments, additional_codesigning_output_path):
    ...
    if codesigning_data.aps_environment is None:
        print('Warning: could not find a valid aps-environment entitlement in the provided provisioning profiles; building without push notification entitlement')
...
def build(bazel, arguments):
    ...
    bazel_command_line.set_sideload_build(arguments.sideloadBuild)
...
if __name__ == '__main__':
    ...
    buildParser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Do not embed application extensions in the IPA.',
    )
    buildParser.add_argument(
        '--sideloadBuild',
        action='store_true',
        default=False,
        help='Build the main app without capabilities that require a paid provisioning profile.',
    )
    buildParser.add_argument(
