...
@objc(AppDelegate) class AppDelegate: UIResponder, UIApplicationDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate, URLSessionDelegate, URLSessionTaskDelegate {
    ...
    private var sideloadLaunchLabel: UILabel?
    ...
    private func urlSession(identifier: String) -> URLSession {
        ...
        if !isCongyugramSideloadBuild {
            ...
            configuration.sharedContainerIdentifier = "group.\(baseAppBundleId)"
        }
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
        ...
        if isCongyugramSideloadBuild {
            let launchLabel = UILabel(frame: hostView.containerView.bounds)
            launchLabel.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            launchLabel.backgroundColor = hostView.containerView.backgroundColor
            launchLabel.textColor = UIColor.systemBlue
            launchLabel.font = UIFont.systemFont(ofSize: 18.0, weight: .semibold)
            launchLabel.textAlignment = .center
            launchLabel.numberOfLines = 0
            launchLabel.text = "Congyugram 正在启动\n1/4 · 初始化窗口"
            hostView.containerView.addSubview(launchLabel)
            self.sideloadLaunchLabel = launchLabel
        ...
        let maybeAppGroupUrl: URL?
        if isCongyugramSideloadBuild {
            maybeAppGroupUrl = nil
        } else {
            maybeAppGroupUrl = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: appGroupName)
        }
        ...
        let applicationBindings = TelegramApplicationBindings(isMainApp: true, appBundleId: baseAppBundleId, appBuildType: buildConfig.isAppStoreBuild ? .public : .internal, containerPath: appGroupUrl.path, appSpecificScheme: buildConfig.appSpecificUrlScheme, openUrl: { url in
           ...
           clearMessageNotifications: { ids in
            ...
            if #available(iOS 10.3, *) {
                let icons = [
                    PresentationAppIcon(name: "CongyuDefault", imageName: "CongyuDefault"),
                    PresentationAppIcon(name: "CongyuVintage", imageName: "CongyuVintage", isPremium: true),
                    PresentationAppIcon(name: "CongyuNeon", imageName: "CongyuNeon", isPremium: true),
                    PresentationAppIcon(name: "CongyuPurpleCat", imageName: "CongyuPurpleCat", isPremium: true),
                    PresentationAppIcon(name: "CongyuCottonCat", imageName: "CongyuCottonCat", isPremium: true),
        ...
        |> deliverOnMainQueue).start(next: { context in
            ...
            if let context = context {
                ...
                |> deliverOnMainQueue).start(next: { _ in
                    ...
                    self.sideloadLaunchLabel?.removeFromSuperview()
                    self.sideloadLaunchLabel = nil
